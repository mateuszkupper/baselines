...
def learn(env,
    ...
    **network_kwargs
    ...
    state_rnn = rnn_init_state(self.rnn)
    reset = True
    state, state_rnn = encode_obs(env.reset(), state_rnn, np.array([0, 0, 1, 0]))
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        for t in range(total_timesteps):
            ...
            action = act(np.array(state)[None], update_eps=update_eps, **kwargs)[0]
            env_action = action
            reset = False
            new_obs, rew, done, _ = env.step(env_action)
            next_state, state_rnn = self.encode_obs(new_obs, state_rnn, env_action)
...
def encode_obs(self, obs, prev_state, action):
    ...
    mu = result[0]
    h = rnn_output(prev_state, mu, 4)
    next_state = rnn_next_state(self.rnn, mu, np.array(action), prev_state)
    return np.concatenate([h, mu]), next_state
