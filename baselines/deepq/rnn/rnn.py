...
def default_hps():
  return HyperParams(num_steps=2000, # train model for 2000 steps.
                     max_seq_len=200, # train on sequences of 100
                     input_seq_width=12,    # width of our data (32 + 3 actions)
                     output_seq_width=8,    # width of our data is 32
                     rnn_size=12,    # number of rnn cells
...
class MDNRNN():
  ...
  def build_model(self, hps):
    ...
    self.input_x = tf.placeholder(dtype=tf.float32, shape=[1, None, 12])
    self.output_x = tf.placeholder(dtype=tf.float32, shape=[1, None, 8])
    ...
    def tf_lognormal(y, mean, logstd):
      return -0.5 * ((tf.concat([y]*8, axis=0) - mean) / tf.exp(logstd)) ** 2 - logstd - logSqrtTwoPI
    ...
    flat_target_data = tf.reshape(self.output_x,[-1, 8])
...
def sample_sequence(sess, s_model, hps, init_z, actions, temperature=1.0, seq_len=1000):
  ...
  for i in range(seq_len):
    input_x = np.concatenate((prev_x, actions[i].reshape((1, 1, 4))), axis=2)
...
def rnn_next_state(rnn, z, a, prev_state):
  input_x = np.concatenate((z.reshape((1, 1, 8)), a.reshape((1, 1, 4))), axis=2)
